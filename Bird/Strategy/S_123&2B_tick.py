...
def init(context):
    ...
    bird.S = get_dominant_future('TA')   # 合约名
    bird.Cout = 200        # 数据条数
    bird.Freq = '1m'        # 数据的频率
...
def handle_tick(context, tick):
    ...
    if CurTime % 5 == 0 and CurTime != bird.Time:
        bird.Time = CurTime # 每间隔5分钟进一起
        ...
        Signal = bird.T.Strategy_2B(DOHLCT)
        
        if Signal[0] == bird.T.Buy:
            ...
            if context.portfolio.positions[bird.S].buy_quantity == 0:
                ...
                pass
            bird.BuyStop = Signal[1]
        if Signal[0] == bird.T.Sell:
            if context.portfolio.positions[bird.S].closable_buy_quantity > 0:
                ...
                print("趋势止损 %s %s",(context.portfolio.positions[bird.S].buy_avg_open_price, tick.last))
            if context.portfolio.positions[bird.S].sell_quantity == 0:
                # sell_open(bird.S,1,style=MarketOrder())
                pass
            bird.SellStop = Signal[1]
        
    if context.portfolio.positions[bird.S].closable_sell_quantity > 0 and tick.last > bird.SellStop:
        buy_close(context.portfolio.positions[bird.S].order_book_id, 1, style=MarketOrder())
    if context.portfolio.positions[bird.S].closable_buy_quantity > 0 and tick.last < bird.BuyStop:
        sell_close(context.portfolio.positions[bird.S].order_book_id, 1, style=MarketOrder())
        print("强止损 %s %s",(context.portfolio.positions[bird.S].buy_avg_open_price, tick.last))
